...
function ChatWindow({ user, activeChat, users }) {
    ...
    try {
      if (activeChat.type === 'personal' && activeChat.user?.id) {
        socket.on('message:send', (msg) => {
          if (msg?.sender === activeChat.user.id || msg?.recipient === user.id) {
  ...
  const isGroupCreator = activeChat.type === 'group' && activeChat.group.creator === user.id;
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
        {messages.map((msg, index) => (
          <div
            key={msg.id || index}
            className={`mb-3 flex ${msg.sender === user.id ? 'justify-end' : 'justify-start'}`}
          >
            <div
